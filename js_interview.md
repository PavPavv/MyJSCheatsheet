...
вернут _false_. Их тоже 7: **false**, **0**, **-0**, **''**, **undefined**, **null** и **NaN**.
...
При сравнении **null** и **undefined** мы получаем _true_, когда используем оператор "==", и _false_ при использовании оператора "==="
(сравнение по типу).
...
Причина такого поведения заключается в том, что name временно преобразуется в объект. У каждого примитива, кроме null и undefined, есть объект-обертка. Такими объектами являются String, Number, Boolean, Symbol и BigInt. Временный объект отбрасывается по завершении работы со свойством или методом.
...
2. Единственное отличие **apply** от **call** состоит в том, что в **apply** аргументы передаются в виде массива.
...
3. **bind** возвращает новую функцию, значением *this* которой является объект, указанный в качестве первого параметра. В отличие от **bind**, **call** и **apply** сразу же вызывают функцию.
```javascript
...
3. _timeout_ появляется последним, потому что это макрозадача.
