...
#### O(log n) - логорифмическая сложность алгоритма

Классов вычислительной сложности в теории бесконечно много, но на практике эти восемь встречаются чаще всего. Константное время выполнения **О(1)** - это низший класс сложности, при этом время работы алгоритма постоянно и не за­висит от размера входных данных. Следующий класс в иерархии - **O(log N)**, логарифмический, мы уже видели, что двоичный поиск относится к этой категории. Оба класса **долинейные**, они содержат чрезвычайно эффективные алгоритмы.
...
#### Асимптоматический анализ

Понятие аддитивной постоянной часто встречается в обыденной жизни, напри­мер, в нашем недавнем разговоре про спидометры. Когда мы говорим «приеду минут через сорок, плюс-минус пять минут», мы имеем в виду как раз аддитив­ную постоянную.

В асимптотическом анализе эта идея развивается и вводится понятие мулъти­пликативной постоянной алгоритма. Кто слышал о законе Мура, уже представ­ляет себе, в чем оно состоит. Еще в 1965 году Гордон Мур, один из основателей Intel, высказал предположение, что и генеральных директоров корпорации количество элементов в интегральных микросхемах будет удваиваться ежегодно в течение ближайших десяти лет. В 1975-м он его скорректировал: «количество элементов в интегральных микросхемах удваивается каждые два года». Это на­блюдение остается верным более 40 лет, и пока оно верно, скорость работы ком­пьютеров тоже удваивается примерно за два года. Такая «мультипликативпая постоянная в истории вычислительной техники» означает, что одна и та же программа на старом компьютере будет работать в тысячу раз медленнее (а то и хуже), чем на новом.
...
Ассоциативный массив - это множество пар "ключ: значение".

Единственное обязательное свойство хеш-функции - однозначность. Хеш, посчитанный от двух одинаковых объектов, должен совпадать; нельзя каж­дый раз брать первое попавшееся число. Хеш неизменяемого объекта (напри­мер, string в Python) можно вычислять лишь единожды и потом хранить - это снизит общее количество вычислений.

### Список основных базовых алгоритмов

1. Числа Фибоначчи (рекурсия и кэш)
2. Анализ верно закрытых скобок
3. Счетчик через замыкание
4. Сумма листьев бинароного дерева
5. Поиск суммы значения во вложенной структуре (словарь со список словарей)
