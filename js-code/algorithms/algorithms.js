...
function first() {
  ...
  // console.log(popularSymbolOpt('ababazzzz aaaasdf'));
  // console.log('popularSymbolOpt',popularSymbolOpt(bigStr));
  // console.log('popularSymbolOpt', funcSpeed(popularSymbolOpt(bigStr)));
...
function dictionaries() {
  ...
  //  console.log(isEqualDigits(2021, 1202)); //  true

  function countBeatingRooks(rooksCoords) {
    const rooksInRow = {};
    const rooksInCol = {};
    const [rowRooks, colRooks] = rooksCoords; //  row = [[0,2]], col = [[2,2]]
    ...
    function iterateRooks(rooks, obj) {
      for (rook of rooks) {
        addRook(obj, rook);
      }
    }

    iterateRooks(rowRooks, rooksInRow);
    iterateRooks(colRooks, rooksInCol);
    return (countPairs(rooksInRow) + countPairs(rooksInCol)) / 2;
  }
  ...
  function groupWords(words) {
    ...
    for (sorted in groups) {
      if (groups[sorted].length < 1) delete groups[sorted];
    }
