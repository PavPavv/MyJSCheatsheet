...
## 2. Назовите полезные методы для работы со свойствами объектами
...
## 4. Пример нативного модуля (через замыкание)
...
## 5. Какая разница между лексической ОВ (JS) и динамической (Perl и Emacs Lisp, нет в JS, похожий концепт у this)?
...
## 6. В чем главное отличие ОВ от this?
...
## 8. Что такое стек вызовов (call stack) и как он работает в V8?
...
## 9. 4 правила определения точки вызова контекста (this)
...
## 10. Что такое прототипы и прототипное наследование (прототипирование)
...
## 11. Что происходит "под капотом" у Object.create()
...
## 13. Что такое внутреннее [[Prototype]] свойство?
...
__proto__ - это геттер/сеттер для **Object.prototype**.
```javascript
...
## 15. Удалит ли оператор delete в объекте свойство, которое тот наследует от другого объекта?
...
## 18. Известные методы http(s)-запросов?

GET, POST, PUT, PATCH, DELETE, TRACE.

## 19. В чем разница между POST, PUT и PATCH?
...
## 21. Как происходит процесс рендера в браузере?
...
## 22. Более подробный разбор ключевых этапов работы браузера
...
1) Parsing HTML to construct the DOM tree -> 2) Render that tree -> 3) Layout of the render tree -> 4) Layout of the render tree 
То же самое, но чуть подробнее:
1) Модуль отображения выполняет синтаксический анализ HTML-документа и переводит теги в узлы DOM в дереве содержания. Информация о стилях извлекается как из внешних CSS-файлов, так и из элементов style.
...
3) После создания дерева отображения начинается компоновка элементов, в ходе которой каждому узлу присваиваются координаты точки на экране, где он должен появиться. 
4) Затем выполняется отрисовка, при которой узлы дерева отображения последовательно отрисовываются с помощью исполнительной части пользовательского интерфейса.
