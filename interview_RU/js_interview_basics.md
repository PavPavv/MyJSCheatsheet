...
## 6. В чем разница между spread-оператором и rest-оператором?
...
## 10. Как в JS скопировать объект?
...
Значение **this** вычисляется во время выполнения кода и зависит от контекста выполнения. При вызове без объекта: **this == undefined**. Стрелочные функции особенные: у них нет своего «собственного» **this**. Если мы используем **this** внутри стрелочной функции, то его значение берётся из внешней «нормальной» функции.
...
Проблему потери идентификатора, указывающего на данные  , в зависимости от условий выполненния кода (**this**) можно решить через замыкание (self = **this**) или с помощью встроенных методов **call**, **apply**, **bind**.
...
## 29. Почему функции в JS - объекты первого класса (First-class Objects)
...
- XML — используется в SOAP (Simple Object Access Protocol) (всегда) и REST (Representational State Transfer)-запросах (реже)
